...
class MinHeap:
    ...
    def __init__(self,ARR=[],flag=True):
        ...
        if flag:
            ...
            self.indeces = {x:-1 for x in range(self.size)}
        else:
            self.heap =  []
            self.size = 0
            self.indeces ={} 
    ...
    def swap(self,i,j):
        ...
        self.indeces[self.heap[i][0]] = i
        self.indeces[self.heap[j][0]] = j
    ...
    def pubbleUpRec (self,i=0):
        ...
        if j < 0:
            return
        if self.heap[i][2] < self.heap[j][2]:
            self.swap(i,j)
            self.pubbleUpRec(j)
    ...
    def pubbleDownRec (self,i=0):
        ...
        if (j1 < self.size)  and (self.heap[i][2] > self.heap[j1][2]):
            if (j2 < self.size)  and(self.heap[j1][2] > self.heap[j2][2]):
                self.swap(i,j2)
                self.pubbleDownRec(j2)
            else:
                self.swap(i,j1)
                self.pubbleDownRec(j1)               
        elif (j2 < self.size)  and(self.heap[i][2] > self.heap[j2][2]):
            self.swap(i,j2)
            self.pubbleDownRec(j2)
        else:return
